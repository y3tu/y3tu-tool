...
import com.y3tu.tool.cache.layering.AbstractValueAdaptingCache;
import com.y3tu.tool.cache.layering.setting.SecondaryCacheSetting;
import com.y3tu.tool.cache.layering.support.ThreadTaskUtils;
...
public class RedisCache extends AbstractValueAdaptingCache {
    ...
    public void evict(Object key) {
        RedisCacheKey redisCacheKey = getRedisCacheKey(key);
        log.info("redis缓存 key= {} 清除缓存", redisCacheKey.getKey());
    ...
    public void clear() {
        ...
        if (usePrefix) {
            log.debug("redis缓存 ，除前缀为{}的缓存", getName());
    ...
    private <T> T loaderAndPutValue(RedisCacheKey key, Callable<T> valueLoader, boolean isLoad) {
        ...
        try {
            ...
            log.debug("redis缓存 key={} 执行被缓存的方法，并将其放入缓存, 耗时：{}。数据:{}", key.getKey(), timeInterval.intervalMs(), JsonUtil.toJson(result));

            if (isLoad && isStats()) {
                getCacheStats().addCachedMethodRequestTime(timeInterval.intervalMs());
            }
            return (T) fromStoreValue(result);
        } catch (Exception e) {
