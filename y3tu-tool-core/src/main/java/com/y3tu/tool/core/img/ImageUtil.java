...
public class ImageUtil {
    ...
    public static void convert(File srcImageFile, File destImageFile) throws IOException {
        ...
        try {
            ...
            convert(ImageIO.read(srcImageFile), destExtName, imageOutputStream, StringUtils.equalsIgnoreCase(IMAGE_TYPE_PNG, srcExtName));
        } catch (IOException e) {
            throw new IORuntimeException(e);
        } finally {
    ...
    public static BufferedImage toImage(String base64) throws IORuntimeException {
        byte[] decode = Base64Util.decode(base64.getBytes());
    ...
    public static Color getColor(String colorName) {
        ...
        if ("BLACK".equals(colorName)) {
            ...
            // rgb值
            final List<String> rgb = ListUtil.toList(StringUtils.split(colorName, ','));
            if (3 == rgb.size()) {
                final Integer r = Integer.valueOf(rgb.get(0));
                final Integer g = Integer.valueOf(rgb.get(1));
                final Integer b = Integer.valueOf(rgb.get(2));
