...
import com.y3tu.tool.core.date.format.DateParser;
import com.y3tu.tool.core.date.format.DatePrinter;
import com.y3tu.tool.core.date.format.FastDateFormat;
...
public class DateUtil {
    ...
    public static int timeToSecond(String timeStr) {
        ...
        final List<String> hms = ListUtil.toList(StringUtils.split(timeStr, String.valueOf(StringUtils.C_COLON), 3));
    ...
    public static List<DateTime> rangeToList(Date start, Date end, final DateField unit) {
        return ListUtil.newArrayList((Iterable<DateTime>) range(start, end, unit));
    }
    ...
    private static String normalize(String dateStr) {
        ...
        // 日期时间分开处理
        final List<String> dateAndTime = ListUtil.toList(StringUtils.split(dateStr, ' '));
