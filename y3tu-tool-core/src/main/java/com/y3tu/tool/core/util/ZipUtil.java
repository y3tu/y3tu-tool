...
public class ZipUtil {
    ...
    public static File zip(File srcFile, Charset charset) throws UtilException {
        File zipFile = FileUtil.file(srcFile.getParentFile(), FilePathUtil.getNameWithoutExtension(srcFile) + ".zip");
    ...
    public static File unzip(File zipFile, Charset charset) throws UtilException {
        return unzip(zipFile, FileUtil.file(zipFile.getParentFile(), FilePathUtil.getNameWithoutExtension(zipFile)), charset);
    }
    ...
    public static byte[] gzip(File file) throws UtilException {
        ...
        try {
            gos = new GZIPOutputStream(bos);
            in = new BufferedInputStream(FileUtil.asInputStream(file));
            IOUtil.copy(in, gos);
     ...
     */
    private static ZipOutputStream getZipOutputStream(File zipFile, Charset charset) throws IOException {
        return getZipOutputStream(FileUtil.asOututStream(zipFile), charset);
    }
    ...
    private static void zip(File file, String srcRootDir, ZipOutputStream out) throws UtilException {
        ...
        final String subPath = FilePathUtil.subPath(srcRootDir, file); // 获取文件相对于压缩文件夹根目录的子路径
        if (file.isDirectory()) {// 如果是目录，则压缩压缩目录中的文件或子目录
            final File[] files = file.listFiles();
            if (ArrayUtil.isEmpty(files) && StringUtils.isNotEmpty(subPath)) {
    ...
    private static void validateFiles(File zipFile, File... srcFiles) throws UtilException {
        for (File srcFile : srcFiles) {
            ...
            if (false == srcFile.exists()) {
                throw new UtilException(StringUtils.format("File [{}] not exist!", srcFile.getAbsolutePath()));
            }
    ...
    private static void copy(ZipFile zipFile, ZipEntry zipEntry, File outItemFile) throws IOException {
        ...
        try {
            ...
            out = FileUtil.asOututStream(outItemFile);
            IOUtil.copy(in, out);
        } finally {
            IOUtil.close(out);
            IOUtil.close(in);
