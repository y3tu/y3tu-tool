...
public class ClassUtil {
    ...
    public static Set<String> getDeclaredMethodNames(Class<?> clazz) {
        return ReflectionUtil.getMethodNames(clazz);
    }
     ...
     *                               例如：com.xiaoleilu.hutool.StringUtils.isEmpty 或 com.xiaoleilu.hutool.StringUtils#isEmpty
     ...
     *
     * @param <T>                     对象类型
     * @param classNameWithMethodName 类名和方法名表达式，例如：com.xiaoleilu.hutool.StringUtils#isEmpty或com.xiaoleilu.hutool.StringUtils.isEmpty
    ...
    public static <T> T invoke(String classNameWithMethodName, boolean isSingleton, Object... args) {
        if (StringUtils.isBlank(classNameWithMethodName)) {
            throw new ReflectionException("Blank classNameDotMethodName!");
        }
        ...
        if (splitIndex <= 0) {
            throw new ReflectionException("Invalid classNameWithMethodName [{}]!", classNameWithMethodName);
        }
    ...
    public static <T> T invoke(String className, String methodName, boolean isSingleton, Object[] args) {
        ...
        try {
            ...
            if (isStatic(method)) {
                ...
                return ReflectionUtil.invoke(isSingleton ? Singleton.get(clazz) : clazz.newInstance(), method, args);
            }
        } catch (Exception e) {
    ...
    public static String getClassPath(boolean isEncoded) {
        ...
        return FilePathUtil.normalize(url);
    }
    ...
    public static URL getResourceURL(String resource) throws IORuntimeException {
        return ResourceUtil.asUrl(resource);
    }
}