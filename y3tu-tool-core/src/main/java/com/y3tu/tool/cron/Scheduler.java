...
import com.y3tu.tool.core.thread.ExecutorBuilder;
import com.y3tu.tool.core.thread.ThreadFactoryBuilder;
import com.y3tu.tool.core.util.CharUtil;
import com.y3tu.tool.core.util.StrUtil;
import com.y3tu.tool.cron.listener.TaskListener;
import com.y3tu.tool.cron.listener.TaskListenerManager;
import com.y3tu.tool.cron.pattern.CronPattern;
import com.y3tu.tool.cron.task.InvokeTask;
import com.y3tu.tool.cron.task.RunnableTask;
        ...
        if (CollectionUtil.isNotEmpty(cronSetting)) {
            ...
            for (Entry<String, LinkedHashMap<String, String>> groupedEntry : cronSetting.getGroupedMap().entrySet()) {
                ...
                for (Entry<String, String> entry : groupedEntry.getValue().entrySet()) {
                    ...
                    log.debug("Load job: {} {}", pattern, jobClass);
                    try {
