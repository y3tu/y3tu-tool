...
public class MongoFactory {
    ...
    public static MongoDS getDS(String... groups) {
        final String key = ArrayUtil.join(groups, GROUP_SEPRATER, null, null);
        MongoDS ds = dsMap.get(key);
        if (null == ds) {
            // 没有在池中加入之
    ...
    public static MongoDS getDS(Setting setting, String... groups) {
        final String key = setting.getSettingPath() + GROUP_SEPRATER + ArrayUtil.join(groups, GROUP_SEPRATER, null, null);
        MongoDS ds = dsMap.get(key);
        if (null == ds) {
            // 没有在池中加入之
