package com.y3tu.tool.db.nosql.mongo;
...
public class MongoDS implements Closeable {
    ...
    synchronized public void initSingle() {
        ...
        try {
            ...
            throw new DbRuntimeException(StringUtils.format("Init MongoDB pool with connection to [{}] error!", serverAddress), e);
        }
    ...
    synchronized public void initCloud() {
        ...
        try {
            mongo = new MongoClient(addrList, buildMongoClientOptions(StringUtils.EMPTY));
        } catch (Exception e) {
    ...
    private ServerAddress createServerAddress(String group) {
        if (setting == null) {
            throw new DbRuntimeException(StringUtils.format("Please indicate setting file or create default [{}], and define group [{}]", MONGO_CONFIG_PATH, group));
        }
        ...
        if (StringUtils.isBlank(tmpHost)) {
            throw new UtilException("Host name is empy of group: " + group);
        }
    ...
    private Builder buildMongoClientOptions(Builder builder, String group) {
        ...
        if (group == null) {
            ...
            group = group + StringUtils.DOT;
        }

        //每个主机答应的连接数（每个主机的连接池大小），当连接池被用光时，会被阻塞住
        Integer connectionsPerHost = setting.getInt("connectionsPerHost", group);
        if (StringUtils.isBlank(group) == false && connectionsPerHost == null) {
            connectionsPerHost = setting.getInt("connectionsPerHost", group);
        }
        ...
        Integer threadsAllowedToBlockForConnectionMultiplier = setting.getInt("threadsAllowedToBlockForConnectionMultiplier", group);
        if (StringUtils.isBlank(group) == false && threadsAllowedToBlockForConnectionMultiplier == null) {
            threadsAllowedToBlockForConnectionMultiplier = setting.getInt("threadsAllowedToBlockForConnectionMultiplier", group);
        }
        ...
        Integer connectTimeout = setting.getInt("connectTimeout", group);
        if (StringUtils.isBlank(group) == false && connectTimeout == null) {
            setting.getInt("connectTimeout", group);
        }
        ...
        Integer socketTimeout = setting.getInt("socketTimeout", group);
        if (StringUtils.isBlank(group) == false && socketTimeout == null) {
            setting.getInt("socketTimeout", group);
        }
