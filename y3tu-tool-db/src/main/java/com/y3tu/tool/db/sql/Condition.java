...
public class Condition implements Cloneable {
    ...
    public String toString() {
        return StringUtils.format("`{}` {} {}", this.field, this.operator, this.value);
    }
    ...
    private void parseValue() {
        ...
        if (StringUtils.equalsIgnoreCase("= null", valueStr) || StringUtils.equalsIgnoreCase("is null", valueStr)) {
            this.operator = OPERATOR_IS;
            this.value = VALUE_NULL;
            return;
        }

        List<String> strs = ListUtil.toList(StringUtils.split(valueStr, String.valueOf(StringUtils.C_SPACE), 2));
        if (strs.size() < 2) {
            return;
        }
        ...
        if (OPERATOR_BETWEEN.equals(firstPart)) {
            ...
            final List<String> betweenValueStrs = ListUtil.toList(StringUtils.split(strs.get(1), LogicalOperator.AND.toString(), 2));
