...
public class Wrapper {
    ...
    public String wrap(String field) {
        ...
        //如果已经包含包装的引号，返回原字符
        if (StringUtils.isSurround(field, preWrapQuote, sufWrapQuote)) {
            return field;
        }

        //如果字段中包含通配符或者括号（字段通配符或者函数），不做包装
        if (StringUtils.containsAnyIgnoreCase(field, "*", "(", " ", "as")) {
            return field;
        }

        //对于Oracle这类数据库，表名中包含用户名需要单独拆分包装
        if (field.contains(StringUtils.DOT)) {
            final Collection<String> target = CollectionUtil.editor(StringUtils.splitToList(field, StringUtils.C_DOT), new Editor<String>() {
                @Override
                public String edit(String t) {
                    return StringUtils.format("{}{}{}", preWrapQuote, t, sufWrapQuote);
                }
            });
            return CollectionUtil.join(target, StringUtils.DOT);
        }

        return StringUtils.format("{}{}{}", preWrapQuote, field, sufWrapQuote);
    }
