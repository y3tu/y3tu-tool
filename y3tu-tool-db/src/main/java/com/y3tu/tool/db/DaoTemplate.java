...
import com.y3tu.tool.db.handler.EntityHandler;
import com.y3tu.tool.db.handler.EntityListHandler;
...
public class DaoTemplate {
	...
	public int update(Entity entity) throws SQLException {
		...
		if (null == pk) {
			throw new SQLException(StringUtils.format("Please determine `{}` for update", primaryKeyField));
		}
	...
	public List<Entity> findBySql(String sql, Object... params) throws SQLException {
		String selectKeyword = StringUtils.subPre(sql.trim(), 6).toLowerCase();
	...
	private Entity fixEntity(Entity entity){
		...
		}else if(StringUtils.isBlank(entity.getTableName())){
