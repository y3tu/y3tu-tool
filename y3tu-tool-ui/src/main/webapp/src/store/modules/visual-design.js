import {deepClone, swap} from '@/utils'
...
const actions = {
    undo({commit, state}) {
        if (state.snapshotIndex >= 0) {
            state.snapshotIndex--
            commit('setComponentData', deepClone(state.snapshotData[state.snapshotIndex]))
        }
    },
...
const mutations = {
    ...
    setComponentData(state, componentData = []) {
        // createApp().set(state, 'componentData', componentData)
